...
def test_ruecksprache_team_participant():
    assert _ruecksprache_team_participant("Rücksprache Andreas") == "Andreas Krause"
    assert _ruecksprache_team_participant("rücksprache armin") == "Armin Bachmann"
    assert _ruecksprache_team_participant("Ruecksprache Fritz") == "Fritz Titzmann"
...
def test_ruecksprache_shorten_removes_overlap(monkeypatch):
    ...
    # Only return the RS appointment for slots that overlap 9:00-10:00
    def fake_iter(folder, start, end, ignore_ids=None):
        if start < datetime(2026, 4, 20, 10, 0) and end > datetime(2026, 4, 20, 9, 0):
            return iter([rs_appt])
        return iter([])

    monkeypatch.setattr("outlook_find_appointment_slot.iter_overlapping_appointments", fake_iter)
...
def test_find_best_slots_no_ruecksprache_moves_when_flag_false(monkeypatch):
    ...
    monkeypatch.setattr("outlook_find_appointment_slot._get_freebusy_string",
                        lambda name, base_date, slot_minutes: freebusy)
    monkeypatch.setattr("outlook_find_appointment_slot.iter_overlapping_appointments",
                        lambda *a, **kw: iter([rs_appt]))
