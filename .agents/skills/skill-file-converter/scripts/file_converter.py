"""File Converter: lokale Dateien nach PDF (COM) oder Markdown (lightrag LLM).

Usage:
    python .agents/skills/skill-file-converter/scripts/file_converter.py to-markdown INPUT [OUTPUT] [--no-llm-pdf] [--all-sheets]
...
def _to_markdown(input_path: Path, output_path: Path, *, no_llm_pdf: bool = False, all_sheets: bool = False) -> int:
...
def _to_pdf_subprocess(input_path: Path, pdf_path: Path, app_type: str) -> None:
    ...
    cmd = [
        ...
        (
            ...
            "from file_converter import _com_to_pdf;"
            "from pathlib import Path;"
...
def main() -> int:
    ...
    # to-markdown
    md = sub.add_parser("to-markdown", help="Datei nach Markdown konvertieren (LLM via lightrag)")
    ...
    md.add_argument("--no-llm-pdf", action="store_true", help="PDF ohne LLM extrahieren")
    ...
    if args.command == "to-markdown":
        ...
        return _to_markdown(input_path, output_path, no_llm_pdf=args.no_llm_pdf, all_sheets=args.all_sheets)
