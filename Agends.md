...
- Abschluss immer mit Referenzübersicht: [3] [Document Title Three](URL#page=X), [Attachment Title](URL#page=X)
...
2) Tool-Ergebnis als JSON parsen
...
- `rag_retrieve` liefert strukturierte Retrieval-Daten (JSON), keine finale Antwort
...
- `get_document_content` / `get_attachment_content` liefern JSON mit `documents[]`
...
```json
{"tool":"rag_retrieve","arguments":{"question":"Welche Änderungen gibt es am Stammdatenservice?","knowledgebase":"default","mode":"mix","max_chunks":20}}
```
...
- Parse das Tool-Result als JSON.
...
Zweck:
- Dieses Tool sucht relevante Informationen in der Knowledge Base und liefert ein strukturiertes JSON-Ergebnis (`rag-result.v2`) zurück.
...
Typischer Workflow:
1) `rag_retrieve` aufrufen, um Kontext + Referenzen zu erhalten.
2) `references` aus dem Ergebnis auswerten.
3) Optional:
   - `get_document_content` für vollständigen Dokumentinhalt / Zeilenausschnitt
   - `list_attachments` für Anhänge eines Parent-Dokuments
   - `get_attachment_content` für Inhalt eines konkreten Anhangs
4) Die finale Antwort mit den gelieferten `chunks` + `instruction` erzeugen.
...
- JSON mit u. a. folgenden Feldern:
...
Wenn du `rag_retrieve` benutzt (Pflicht):
- Parse das Tool-Result als JSON.
- Befolge die Instruktion im Feld `instruction`.
- Nutze primär `chunks` (plus `entities`, `relationships`, `references`) als Wissenskontext für deine finale Antwort.
- Werte `coverage_gaps`, `is_enough_for_answer` und `recommended_next_actions` aus, falls vorhanden.
- Wenn `is_enough_for_answer=false`, führe mindestens einen weiteren Tool-Schritt aus.
"""
